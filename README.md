...
## 界面快照（本地 Web 控制台，浏览器打开 http://127.0.0.1:8787）

| | |
|---|---|
| 登录步骤 1：token 验证 | 登录步骤 2：API 厂家/模型切换 |
| ![login-token](docs/screens/01-login-token.png) | ![login-provider](docs/screens/02-login-provider.png) |
| 会话主界面（流式消息 + 工具卡） | 权限请求弹窗（本次/总是/拒绝 + always 级联） |
| ![console](docs/screens/03-console-main.png) | ![permission](docs/screens/06-permission.png) |
| Skills 面板（热加载监视状态） | 任务完成态（Write→Read 工具链 + 逐轮 token 用量） |
| ![skills](docs/screens/04-skills-panel.png) | ![final](docs/screens/07-final.png) |
...
| **Skills** | SKILL.md + frontmatter，分层扫描（全局←项目覆盖）；双轨热加载（WatchService 递归注册 + 2s mtime 指纹兑底，含活跃会话 workdir 动态根）；per-root 快照缓存与缓存契约（内容 equals 引用短路，热加载不扰动 prompt cache 前缀）；扫描失败可见化；`/api/skills` 观测端点 + CLI `/skills [reload]`，详见 [docs/03-方案-Skills热加载增强](docs/03-方案-Skills热加载增强.md) |
...
| **Web 控制台** | Spring MVC + SSE（有界队列/critical 不丢/Last-Event-ID 重放，首帧即时 flush）+ Vue3（CDN 本地化免构建）：两步登录（token 验证 → API 厂家/模型选择，写 `chat.default` 下一轮热生效）；会话重命名（他端改名经 SSE 实时同步）；状态/Todos/Tasks/工具/Skills/模型多面板；权限/提问双端先到先得 |
...
| M9 | 控制台增强：两步登录/厂家切换、会话重命名、Skills 热加载增强（per-root 快照 + 缓存契约 + 动态根 + 观测端点）、SSE 首帧/去重修复 | ✅ 2026-09 |
...
**Web 控制台**
- 两步登录：token 验证遮罩 → API 厂家/模型选择（`GET/POST /api/providers/default`，写回 `common.chat.default` 热生效；缺 Key 厂家可页内补录，选中隐含启用）
- 修复会话列表/历史消息不显示：前后端 `{items}` 响应契约对齐（旧前端按裸数组解析）
- 会话重命名：列表项 hover ✎ 行内编辑，Enter 提交；他端（CLI/其它窗口）改名经 `session.updated` 实时同步
- 新增 Skills / Tasks / Todos 面板与对应后端端点（`/api/skills`、`/api/sessions/{id}/todos`、`/api/sessions/{id}/tasks`）
- SSE：订阅即推首帧（onopen 由心跳级 ~16s 降至 <1s）；乐观消息与持久化消息双显修复；用户消息 role 渲染修复（订阅 `message.updated`）
- 前端依赖 CDN 本地化（`static/vendor/`），离线可用

**Skills 热加载增强**（方案与验收记录见 [docs/03](docs/03-方案-Skills热加载增强.md)）
- per-root 快照缓存：多会话多项目互不覆写（修复串扰），内容 equals 引用短路 → 热加载零扰动 prompt cache
- 动态监视根：活跃会话 workdir 的 `.we0j/skills` 纳入双轨监视
- 扫描失败可见化（ScanMeta.failed → API/面板/CLI）；CLI `/skills [reload]` 与 `doctor` skills 检查项
- 新增单测：C2 引用短路 / C3 串扰隔离 / 动态根置脏 / 失败可见（SkillPerRootSnapshotTest）
