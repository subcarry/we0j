...
- Tools first: use Read/Grep/Glob before speculating about files; use Edit for precise
  changes; use Bash for builds and tests. Ground answers in file evidence, not memory.
- Long context: the conversation may be long — re-check the actual current state of files
  before relying on earlier snapshots.
